import logging
from dataclasses import dataclass
from datetime import datetime, timezone
from enum import Enum

from app.core.input_gate_config import InputGateConfig
from app.db.client_models import MoonlightAccount, MoonlightCall
from app.db.models import CallStorage
from app.domain.input_gate import evaluate_input_gate
from app.domain.transcript import Transcript
from app.services.fetcher.repository import filter_new_calls, list_all_client_calls
from app.services.fetcher.transform import (
    TranscriptShapeError,
    build_call_metadata,
    transcript_to_storage_shape,
)

logger = logging.getLogger(__name__)


class FetchOutcome(str, Enum):
    """What happened to one call.

    EXCLUDED still means *stored*: the input gate refused it, so the row carries
    `excluded_reason` and the analyser will never seed an `analysis` row for it.
    Not storing a rejected call was considered and rejected — dedup decides
    novelty by absence from `call_storage`, so it would be re-fetched from Avoma
    on every run forever, leaving no record that we had ever seen it.
    """

    STORED = "stored"
    EXCLUDED = "excluded"
    NO_TRANSCRIPT = "no_transcript"


@dataclass
class FetchSummary:
    candidates: int
    fetched: int
    skipped_no_transcript: int
    skipped_malformed_transcript: int = 0
    excluded: int = 0


def fetch_and_store_call(
    *,
    client_session,
    our_session,
    avoma_client,
    call: MoonlightCall,
    input_gate_config: InputGateConfig | None = None,
) -> FetchOutcome:
    """Fetches one call's transcript from Avoma and writes it to call_storage.

    Returns NO_TRANSCRIPT (no write) if Avoma doesn't have a transcript yet —
    the call simply stays absent from call_storage and is retried automatically
    on the next run; no separate status/retry tracking needed for that case.

    The input gate runs here, before an `analysis` row exists, so a call with no
    assessable conversation in it costs no gateway request. It never prevents the
    write, only marks it.
    """
    transcript = avoma_client.get_transcript_by_meeting_uuid(call.avoma_meeting_uuid)
    if transcript is None:
        return FetchOutcome.NO_TRANSCRIPT

    stored_transcript = transcript_to_storage_shape(transcript)

    verdict = evaluate_input_gate(
        Transcript.model_validate(stored_transcript),
        input_gate_config or InputGateConfig(),
    )
    if not verdict.accepted:
        logger.info(
            "excluding call %s from analysis: %s (%s)",
            call.avoma_meeting_uuid,
            verdict.reason.value if verdict.reason else "unknown",
            verdict.detail,
        )
    elif verdict.client_speech_skipped:
        # Analysed, but one of the two checks could not reach an answer. Logged
        # so a run that abstains often is visible rather than looking clean.
        logger.info(
            "call %s analysed without the client-speech check: %s",
            call.avoma_meeting_uuid,
            verdict.detail,
        )

    account = client_session.get(MoonlightAccount, call.account_id) if call.account_id else None
    our_session.add(
        CallStorage(
            avoma_recording_id=call.avoma_meeting_uuid,
            client_record_id=account.crm_account_id if account else "unknown",
            transcript=stored_transcript,
            call_metadata=build_call_metadata(call, account),
            fetched_at=datetime.now(timezone.utc),
            excluded_reason=verdict.reason.value if verdict.reason else None,
            excluded_detail=verdict.detail if not verdict.accepted else None,
        )
    )
    our_session.commit()
    return FetchOutcome.STORED if verdict.accepted else FetchOutcome.EXCLUDED


def fetch_new_calls(
    *,
    client_session,
    our_session,
    avoma_client,
    limit: int | None = None,
    input_gate_config: InputGateConfig | None = None,
) -> FetchSummary:
    all_calls = list_all_client_calls(client_session)
    new_calls = filter_new_calls(our_session, all_calls)
    if limit is not None:
        new_calls = new_calls[:limit]

    fetched = 0
    excluded = 0
    skipped_no_transcript = 0
    skipped_malformed_transcript = 0
    for call in new_calls:
        try:
            outcome = fetch_and_store_call(
                client_session=client_session,
                our_session=our_session,
                avoma_client=avoma_client,
                call=call,
                input_gate_config=input_gate_config,
            )
        except TranscriptShapeError:
            # One unanchorable or unattributable transcript must not abort the
            # run — the scheduler skips the analyser entirely when the fetcher
            # raises.
            logger.exception(
                "skipping call %s: transcript cannot be anchored or attributed",
                call.avoma_meeting_uuid,
            )
            our_session.rollback()
            skipped_malformed_transcript += 1
            continue

        if outcome is FetchOutcome.STORED:
            fetched += 1
        elif outcome is FetchOutcome.EXCLUDED:
            excluded += 1
        else:
            skipped_no_transcript += 1

    return FetchSummary(
        candidates=len(new_calls),
        fetched=fetched,
        skipped_no_transcript=skipped_no_transcript,
        skipped_malformed_transcript=skipped_malformed_transcript,
        excluded=excluded,
    )
