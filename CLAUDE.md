...
- **`HIGH_THRESHOLD` was raised 4.2 → 4.7 on 2026-08-14; `MEDIUM_THRESHOLD`
  stays at the business team's 2.8.** Excluding N/A removed the low scores that
  dragged every mean down, so means rose while the thresholds stayed put.
  Measured against 43 blind hand grades, **v2 as originally shipped was NOT an
  improvement** — 23/43 against v1's 24/43, with 19 of its 20 errors grading a
  call too generously and 2 opposite-tier errors v1 never made. At 4.7 it scores
  33/43 with **0** opposite-tier errors. Validated on a holdout (+14 and +9
  points in the two fold directions), so ~65–75% is the honest expectation
  rather than the 77% in-sample figure. The winning region is a broad plateau
  (High 4.6–4.8), so don't over-read the decimal. `docs/eval/2026-08-14-call-score-accuracy.md`.
- **A threshold is specific to a category mix.** 4.7 was derived for `v2`'s ten
  categories. Any future prompt version changes the means and needs its own
  threshold derived the same way — do not carry 4.7 across.
...
- **Supplier and partner calls are still analysed as sales calls — but "cannot be detected from a transcript" was WRONG, corrected 2026-08-14.** The original claim held only for *structural* rules: the HelloWork call has a real non-rep speaking 1,898 words on the account's own registered domain (`hellowork.com`), so domain/speaker/word-count signals all read as an ordinary client call, and the input gate's rules genuinely cannot reach it. **Reading the call for meaning detects it immediately.** During the hand-grading of all 47 calls, independent graders flagged **three** calls as Joveo-is-the-buyer (`47` HelloWork, `52` and `58` Talroo) — about 6% of the corpus — with no coordination and no structural cue. So an LLM check ("is the Joveo participant buying or selling here?") is a viable step; it was never tried, only rule-based approaches were. A hand-maintained denylist of job-board domains remains rejected for the original reason. The durable fix is still a buyer/supplier flag on `moonlight_accounts` (Koushik's schema); the LLM check is the interim one.
